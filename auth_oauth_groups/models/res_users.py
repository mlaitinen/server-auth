...
class ResUsers(models.Model):
    ...
    def get_directory_groups(self):
        ...
        :return: A list of identifiers of groups that the user belongs to in an external
         directory.
        """
        return []
    ...
    def _update_groups_from_directory(self):
        ...
        if provider.new_groups_handling == "ignore":
            ...
            # Log a warning
            unknown_groups = set(users_directory_groups) - set(
                new_odoo_group_mappings.mapped("directory_group_id.group_identifier")
            )
            if unknown_groups:
                _logger.warning(
                    f"User '{self.login}' logged in using OAuth provider "
                    f"'{provider.name}' belongs to the following directory groups that "
                    f"don't exist in Odoo: {list(unknown_groups)}"
                )

        existing_odoo_groups = self.groups_id

        # If there are multiple user types (internal, portal, public) defined in the
        # group mappings, fail the login because that would lead to an unresolvable
        # situation.
        ...
        if len(conflicting_groups) > 1:
            ...
            raise ValidationError(
                f"OAuth provider {provider.name} has conflicting group mappings: "
                f"{conflicting_group_names}. A single user can belong to only one of "
                f"the following groups at once: {user_type_group_names}."
            )
        ...
        if provider.force_groups:
                ...
                sudo_self.write(
                    {
                        "groups_id": [
                            (
                                ...
                                0,
                                new_odoo_groups.ids
                                or [self.env.ref("base.group_public").id],
                            )
                        ]
                    }
                )
            ...
            # Make sure that the user only belongs to one of the following groups.
            # This is a restriction set by Odoo. Remove them from the other groups.
            ...
            for user_type_group in user_type_groups:
                ...
                if (
                    user_type_group in sudo_self.groups_id
                    and other_essential_groups & groups_to_add_transitive
                ):
            ...
            sudo_self.write(
                {
                    "groups_id": implicit_removals
                    + [(4, group_id) for group_id in groups_to_add.ids]
                }
            )

        _logger.info(
            f"Added user {self.login} to groups {sorted(groups_to_add.ids)} "
